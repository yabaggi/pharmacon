...
// v2 — D1 only (images stored as base64 BLOBs)
// ============================================================
...
function authUser(req) {
  ...
  return parseToken(h.replace("Bearer ", ""));
}
...
// Handles all D1 BLOB return formats across Wrangler versions
function blobToResponse(img, contentType = "image/jpeg") {
  let body;
...
export default {
  async fetch(req, env) {
    ...
    try {
      ...
      // Image upload & serve (D1 BLOB)
      if (path === "/api/upload"         && req.method === "POST") return uploadImage(req, env);
      if (path.startsWith("/api/image/") && req.method === "GET")  return serveImage(req, env);
      ...
      if (path.startsWith("/api/users/")         && req.method === "DELETE") return deleteUser(req, env);

      // Pharmacists list
      if (path === "/api/pharmacists"            && req.method === "GET")    return listPharmacists(req, env);

      // Messages BEFORE /:id routes to prevent route swallowing
...
// ============================================================
// IMAGE — Upload (store as BLOB in D1 attachments table)
// ============================================================
async function uploadImage(req, env) {
  ...
  if (!allowed.includes(file.type)) return err("Only image files allowed (JPEG, PNG, GIF, WEBP)");

  const maxBytes = 900 * 1024;
  if (file.size > maxBytes) return err("File too large. Max 900KB after compression.");

  const bytes = await file.arrayBuffer();
  const result = await env.DB.prepare(
    "INSERT INTO attachments (uploader_id, content_type, data, size, created_at) VALUES (?,?,?,?,datetime('now'))"
  ).bind(u.id, file.type, bytes, file.size).run();

  const id = result.meta.last_row_id;
  return json({ url: `/api/image/${id}`, id });
}

// ============================================================
// IMAGE — Serve from D1 BLOB
// ============================================================
async function serveImage(req, env) {
  const id = parseInt(new URL(req.url).pathname.split("/api/image/")[1]);
  const row = await env.DB.prepare(
    "SELECT data, content_type FROM attachments WHERE id=?"
  ).bind(id).first();
  if (!row) return new Response("Not found", { status: 404 });
  const res = blobToResponse(row.data, row.content_type);
  if (!res) return new Response("Invalid image data", { status: 500 });
  return res;
}
...
async function register(req, env) {
  ...
  return json({ token: makeToken({ id: user.id, email: user.email, role: user.role }), user });
}
...
async function getUser(req, env) {
  const u = authUser(req);
  if (!u) return err("Unauthorized", 401);
  const id = parseInt(new URL(req.url).pathname.split("/api/users/")[1]);
  const user = await env.DB.prepare(
    "SELECT id,name,name_ar,email,role,license_number,phone,specialty,created_at,is_active FROM users WHERE id=?"
  ).bind(id).first();
...
async function updateUser(req, env) {
  ...
  const { name, name_ar, phone, specialty, license_number } = body;
  await env.DB.prepare(
    `UPDATE users SET name=COALESCE(?,name), name_ar=COALESCE(?,name_ar),
     phone=COALESCE(?,phone), specialty=COALESCE(?,specialty), license_number=COALESCE(?,license_number)
     WHERE id=?`
  ).bind(name||null, name_ar||null, phone||null, specialty||null, license_number||null, id).run();
  if (u.role === "admin") {
    ...
    if (body.password) {
      ...
      await env.DB.prepare("UPDATE users SET password_hash=? WHERE id=?").bind(pw, id).run();
    }
  }
  const updated = await env.DB.prepare(
    "SELECT id,name,name_ar,email,role,phone,specialty,license_number,is_active FROM users WHERE id=?"
  ).bind(id).first();
  return json(updated);
}

async function deleteUser(req, env) {
  const u = authUser(req);
  if (!u || u.role !== "admin") return err("Forbidden", 403);
  const id = parseInt(new URL(req.url).pathname.split("/api/users/")[1]);
  if (u.id === id) return err("Cannot delete your own account");
  await env.DB.prepare("UPDATE users SET is_active=0 WHERE id=?").bind(id).run();
  return json({ ok: true });
}

async function listPharmacists(req, env) {
  const u = authUser(req);
  if (!u) return err("Unauthorized", 401);
  const { results } = await env.DB.prepare(
    "SELECT id,name,name_ar,role,license_number,specialty FROM users WHERE role IN ('community_pharmacist','volunteer_pharmacist') AND is_active=1 ORDER BY name"
  ).all();
  return json(results);
}
...
async function listConsultations(req, env) {
  ...
  if (u.role === "patient") {
    ...
    q = `SELECT c.*, pt.name as patient_name, pt.name_ar as patient_name_ar,
                p.name as pharmacist_name, p.name_ar as pharmacist_name_ar
...
async function createConsultation(req, env) {
  ...
  if (!subject && !subject_ar) return err("Subject required");
  if (!description && !description_ar) return err("Description required");
  const result = await env.DB.prepare(
    `INSERT INTO consultations
       (patient_id,subject,subject_ar,description,description_ar,priority,category,drug_names,status,created_at,updated_at)
     VALUES (?,?,?,?,?,?,?,?,?,datetime('now'),datetime('now'))`
  ).bind(u.id, subject||null, subject_ar||null, description||null, description_ar||null,
    priority||"normal", category||"general", drug_names||null, "open").run();
  const id = result.meta.last_row_id;
  const pharms = await env.DB.prepare(
    "SELECT id FROM users WHERE role IN ('community_pharmacist','volunteer_pharmacist') AND is_active=1"
  ).all();
  for (const ph of pharms.results) {
    await env.DB.prepare(
      ...
      `Patient submitted: ${subject||subject_ar}`, `مريض قدّم: ${subject_ar||subject}`, id).run();
  }
...
async function updateConsultation(req, env) {
  ...
  if (body.assign && ["community_pharmacist","volunteer_pharmacist"].includes(u.role)) {
    await env.DB.prepare(
      "UPDATE consultations SET pharmacist_id=?,status='in_progress',updated_at=datetime('now') WHERE id=?"
    ).bind(u.id, id).run();
    await env.DB.prepare(
      "INSERT INTO notifications (user_id,type,title,title_ar,body,body_ar,ref_id,created_at) VALUES (?,?,?,?,?,?,?,datetime('now'))"
  ...
  if (body.pharmacist_id !== undefined && u.role === "admin") {
    const newPharmId = body.pharmacist_id || null;
    await env.DB.prepare(
      "UPDATE consultations SET pharmacist_id=?,status=?,updated_at=datetime('now') WHERE id=?"
    ).bind(newPharmId, newPharmId ? "in_progress" : "open", id).run();
    if (newPharmId) {
      await env.DB.prepare(
        "INSERT INTO notifications (user_id,type,title,title_ar,body,body_ar,ref_id,created_at) VALUES (?,?,?,?,?,?,?,datetime('now'))"
      ).bind(row.patient_id,"consultation_assigned","Pharmacist Assigned","تم تعيين صيدلاني",
        "A pharmacist has been assigned to your consultation","تم تعيين صيدلاني لاستشارتك", id).run();
      await env.DB.prepare(
        "INSERT INTO notifications (user_id,type,title,title_ar,body,body_ar,ref_id,created_at) VALUES (?,?,?,?,?,?,?,datetime('now'))"
      ).bind(newPharmId,"consultation_assigned","Consultation Assigned to You","تم تعيين استشارة لك",
        "Admin assigned a consultation to you","عيّن المدير استشارة لك", id).run();
    }
  }

  if (body.status) {
    await env.DB.prepare(
      "UPDATE consultations SET status=?,updated_at=datetime('now') WHERE id=?"
  ...
  if (body.rating && u.role === "patient" && row.patient_id === u.id) {
    await env.DB.prepare(
      "UPDATE consultations SET rating=?,rating_comment=? WHERE id=?"
...
async function getMessages(req, env) {
  ...
  const id = parseInt(parts[parts.indexOf("consultations") + 1]);
  const { results } = await env.DB.prepare(
    `SELECT m.id,m.consultation_id,m.sender_id,m.body,m.body_ar,
            m.attachment_id,m.attachment_type,m.created_at,
            u.name as sender_name, u.name_ar as sender_name_ar, u.role as sender_role
     FROM messages m LEFT JOIN users u ON m.sender_id=u.id
     WHERE m.consultation_id=? ORDER BY m.created_at ASC`
  ).bind(id).all();
  const msgs = (results ?? []).map(m => ({
    ...m,
    attachment_url: m.attachment_id ? `/api/image/${m.attachment_id}` : null,
  }));
  return json(msgs);
}
...
async function sendMessage(req, env) {
  ...
  const parts = new URL(req.url).pathname.split("/");
  const id = parseInt(parts[parts.indexOf("consultations") + 1]);
  const { body, body_ar, attachment_id, attachment_type } = await req.json();
  if (!body && !body_ar && !attachment_id) return err("Message body or attachment required");
  ...
  const result = await env.DB.prepare(
    "INSERT INTO messages (consultation_id,sender_id,body,body_ar,attachment_id,attachment_type,created_at) VALUES (?,?,?,?,?,?,datetime('now'))"
  ).bind(id, u.id, body||null, body_ar||null, attachment_id||null, attachment_type||null).run();
  ...
  const msg = await env.DB.prepare(
    `SELECT m.id,m.consultation_id,m.sender_id,m.body,m.body_ar,m.attachment_id,m.attachment_type,m.created_at,
            u.name as sender_name, u.name_ar as sender_name_ar, u.role as sender_role
     FROM messages m LEFT JOIN users u ON m.sender_id=u.id WHERE m.id=?`
  ).bind(result.meta.last_row_id).first();

  return json({ ...msg, attachment_url: msg.attachment_id ? `/api/image/${msg.attachment_id}` : null }, 201);
}
...
async function getPatient(req, env) {
  const u = authUser(req);
  if (!u) return err("Unauthorized", 401);
  const id = parseInt(new URL(req.url).pathname.split("/api/patients/")[1]);
...
async function updatePatient(req, env) {
  ...
  await env.DB.prepare(
    `UPDATE patient_records SET
       date_of_birth=COALESCE(?,date_of_birth), blood_type=COALESCE(?,blood_type),
...
async function getAnalytics(req, env) {
  const u = authUser(req);
  if (!u || !["admin","community_pharmacist","volunteer_pharmacist"].includes(u.role)) return err("Forbidden", 403);
  const [total, open, inprog, resolved, avgRating, patients, messages] = await Promise.all([
  ...
  const byCategory     = await env.DB.prepare("SELECT category,COUNT(*) as count FROM consultations GROUP BY category ORDER BY count DESC").all();
  const daily          = await env.DB.prepare(`SELECT date(created_at) as day,COUNT(*) as count FROM consultations WHERE created_at>=date('now','-30 days') GROUP BY day ORDER BY day`).all();
  const topPharmacists = await env.DB.prepare(`SELECT u.name,u.name_ar,COUNT(c.id) as resolved FROM consultations c JOIN users u ON c.pharmacist_id=u.id WHERE c.status='resolved' GROUP BY c.pharmacist_id ORDER BY resolved DESC LIMIT 5`).all();
  return json({
    totals: { consultations:total.n, open:open.n, in_progress:inprog.n, resolved:resolved.n, avg_rating:avgRating.n, patients:patients.n, messages:messages.n },
...
async function markNotificationRead(req, env) {
  const u = authUser(req);
  if (!u) return err("Unauthorized", 401);
  const id = parseInt(new URL(req.url).pathname.split("/api/notifications/")[1]);
